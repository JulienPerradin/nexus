...
for file in files:
                ...
                if file == filename:
                    with open(os.path.join(subdir,file),'r') as i:
                        for l_index,line in enumerate(i):
                            if l_index == 0 or line[0] == '#':
                                continue
                            else:
                                parts = line.split()
                                if parts[-1] not in readed_data:
                                    readed_data[parts[-1]] = []
                                readed_data[parts[-1]].append([float(parts[0]),float(parts[2]), float(parts[4])])
    else:
        for subdir, dirs, files in os.walk(dir):
            dirs.sort()
            for file in files:
                            ...
                            if l_index == 0 or line[0] == '#':
                                ...
                                if parts[-1] == '1D':
                                    if parts[-3] not in readed_data:
                                        readed_data[parts[-3]] = []
                                    readed_data[parts[-3]].append([float(parts[0]),float(parts[2]),float(parts[4])])
    ...
    keys = [
        ...
        ["OSi2-OSi2", "OSi2-OSi3", "OSi3-OSi3", "O2-O2", "O2-O3", "O3-O3"]
    ]
    ...
    for i, key in enumerate(keys):
        for j, k in enumerate(key):
            ax[i//3, i%3].errorbar(np.array(readed_data[k])[:,0], np.array(readed_data[k])[:,1], np.array(readed_data[k])[:,2], label=k, fmt='-o', mec="black", mfc=colours[j], color=colours[j])
            ax[i//3, i%3].fill_between(np.array(readed_data[k])[:,0], np.array(readed_data[k])[:,1]-np.array(readed_data[k])[:,2], np.array(readed_data[k])[:,1]+np.array(readed_data[k])[:,2], alpha=0.2, color=colours[j])
            # if k not in ref_keys:
                # ax[i//3, i%3].errorbar(np.array(readed_data[k][:,0]), np.array(readed_data[k])[:,0], np.array(readed_data[k])[:,1], label=k, fmt='-o', mec="black", mfc=colours[j], color=colours[j])
                # ax[i//3, i%3].fill_between(np.array(readed_data[k][:,0]), np.array(readed_data[k])[:,0]-np.array(readed_data[k])[:,1], np.array(readed_data[k])[:,0]+np.array(readed_data[k])[:,1], alpha=0.2, color=colours[j])
            # else:
                # ax[i//3, i%3].plot(np.array(readed_data[k][:,0]), np.array(readed_data[k])[:,0], '--', label=k, color=colours[j])
            
        ax[i//3, i%3].set_title(str(key).replace(',',' |').replace('[','').replace(']','').replace('\'',''))
        ax[i//3, i%3].set_xlabel(rf"Concentration $\rho$")
        ax[i//3, i%3].set_ylabel(f"{str(filename.split('.')[0]).replace('_',' ').title()}")
